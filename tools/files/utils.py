...
def resize(path, size, new_path=None, crop_data=None):
    if isinstance(crop_data, basestring):
        crop_data = json.loads(crop_data)
    #print 'crop_data: ', crop_data

    # crop - None,center
    profile_path = os.path.join(os.path.dirname(__file__), 'AdobeRGB1998.icc')
    if not new_path:
    ...
    if size.get('custom_cmd'):
        ...
        extent = "{width}x{height}".format(width=min(width, sz[0]), height=min(height, sz[1]))
        ...
        if crop == 'center':
            param = "convert \"%s\" -resize \"%s^\" -gravity center -extent \"%s\" -layers OptimizeTransparency -strip -interlace Plane -quality %d -profile \"%s\" %s\"%s\"" % (
                path, extent, extent, qual, profile_path, add_param, new_path)
        elif crop == 'top':
            param = "convert \"%s\" -resize \"%s^\" -gravity north -extent \"%s\" -layers OptimizeTransparency -strip -interlace Plane -quality %d -profile \"%s\" %s\"%s\"" % (
        ...
        # param = "convert \"%s\" -resize \"%d>x\" -crop \"%dx%d+0+0\" -layers OptimizeTransparency -quality %d %s\"%s\"" % (path, width, width, height, qual, add_param, new_path)
            ...
            param = "convert \"%s\" -resize \"%s>\" -sampling-factor 4:2:0 -layers OptimizeTransparency -strip -interlace Plane -quality %d -profile \"%s\" %s\"%s\"" % (
...
def resize_bulk(directory, filename, sizes, tn_name=None, crop_data=None):
    ...
    for label, size in sizes.items():
        ...
        try:
            os.makedirs(os.path.join(directory, label), 0775)
        except os.error:
            pass
